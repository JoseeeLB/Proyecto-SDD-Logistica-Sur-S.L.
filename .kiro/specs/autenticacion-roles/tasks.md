...
- [x] 4.1 Configurar roles técnicos mínimos y el patrón de equipos por centro
  ...
  - _Boundary:_ Modelo de Seguridad de Centro y Perfil
- [x] 4.2 Materializar el contrato de alcance para specs consumidoras
...
- Nota de correccion 6: tras anadir los origenes de datos en Studio, Power Fx los registro con su **display name** (`Name` en `DataSources.json`), no con el `EntitySetName`: la tabla `jlb_perfilusuario` quedo como origen de datos `'Perfiles de Usuario'` y `jlb_centrotrabajo` como `'Centros de Trabajo'`. La formula `OnStart` seguia referenciando `jlb_perfilusuarios` (el EntitySetName usado antes de que existiera el origen de datos real), lo que producia "El nombre no es valido. No se reconoce 'jlb_perfilusuarios'". Se actualizo `Filter(jlb_perfilusuarios, ...)` a `Filter('Perfiles de Usuario', ...)`. Recordatorio para specs posteriores: tras anadir un origen de datos en Studio, comprobar el nombre real asignado (visible en el panel "Datos" o en `References/DataSources.json`) antes de escribir formulas, en vez de asumir el EntitySetName de Dataverse.
- Tarea 4.1: se crearon tres security roles Dataverse (`Operario Logistica Sur`, `Supervisor Logistica Sur`, `Administrador Logistica Sur`) en la unidad de negocio raiz, con privilegios minimos sobre `jlb_perfilusuario`/`jlb_centrotrabajo`: Operario (lectura local de perfil, lectura global de centro), Supervisor (lectura/escritura/creacion local de perfil, lectura global de centro), Administrador (control total global sobre ambas tablas). El lookup `jlb_teamid` en `jlb_centrotrabajo` (creado en tarea 1.1) establece el patron: cada centro activo puede asociarse a un owner team de Dataverse (creado por el administrador al dar de alta el centro) que sirve de perimetro de datos para specs downstream (ownership/compartición por equipo). No se crearon equipos ni centros de ejemplo porque la tabla de centros esta vacia en este entorno (responsabilidad operativa del administrador al desplegar).
- Tarea 4.2: el `AccessContext` canonico ya expone `dataScope` (self/center/global), `perfilUsuarioId` y `centroSecurityTeamId` desde `OnStart` (implementado en tarea 2.3). Se verifico que el modelo de seguridad Dataverse no otorga acceso masivo cruzado entre Operarios del mismo centro: los security roles operan a nivel `Local`/`Global` por tabla, no por relacion de centro, y el filtrado real por centro/self se realiza en la capa de aplicacion (specs downstream) usando `dataScope`/`centroTrabajoId`/`centroSecurityTeamId` del contrato, no por privilegios Dataverse de equipo compartido a nivel de registro individual de incidencia (esa logica de sharing se implementara en `incidencias-core`).
